...
export function DashboardContent() {
  const activeDeals = MOCK_DEALS.filter(d => d.status === "active").slice(0, 5);
  const todayTasks = MOCK_TASKS.filter(
    t => t.status !== "completed" && t.status !== "cancelled"
  ).slice(0, 5);
  const criticalInsights = MOCK_AI_INSIGHTS.filter(
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      >
        <div>
          <h1 className="text-xl font-bold text-foreground">Good morning, Sarah</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            You have {MOCK_KPI.tasks_due_today} tasks due today and{" "}
            {MOCK_KPI.active_deals} active deals.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          title="Active Deals"
          value={MOCK_KPI.active_deals}
          trend={MOCK_KPI.active_deals_trend}
        ...
        <KPICard
          ...
          value={MOCK_KPI.tasks_due_today}
          trendLabel={`${MOCK_KPI.tasks_overdue} overdue`}
          icon={CheckSquare}
        ...
        <KPICard
          ...
          value={MOCK_KPI.docs_pending_signature}
          trendLabel="across 3 deals"
          icon={PenLine}
        ...
        <KPICard
          ...
          value={MOCK_KPI.closings_this_month}
          trend={MOCK_KPI.closings_trend}
          trendLabel={formatCurrency(MOCK_KPI.closings_revenue, true)}
          icon={TrendingUp}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          ...
                          <span>·</span>
                          <span
                            className={daysToClose <= 7 ? "text-amber-400 font-medium" : ""}
                          >
            ...
            <Card>
              <CardContent className="p-4 space-y-4">
                {MOCK_ACTIVITIES.slice(0, 5).map((activity) => {
                  ...
                  const colorClass = activityColors[activity.color ?? "indigo"] ?? activityColors.indigo;
                  return (
